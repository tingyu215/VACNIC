...
def gen_caption_from_loader_instructblip(model, data_loader, processor, beam_size, max_length, use_opt=True):
    out_dict = {}
    for step, batch in enumerate(tqdm(data_loader, desc="Iteration")):
        ...
        if args.use_retrieval:
            ...
            for i in range(len(raw_img)):
                prompts.append(f"News article:{raw_article[i]}Generate news image caption:")
        else:
...
if __name__ == "__main__":
    ...
    model =InstructBlipForConditionalGeneration.from_pretrained("Salesforce/instructblip-vicuna-7b", quantization_config=quantization_config)
    ...
    with open(f"/OUTDIR/instructblip_{args.data_type}_retrieve{args.use_retrieval}_prompt.json", "w") as f:
